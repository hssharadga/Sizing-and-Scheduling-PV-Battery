...
function [u_opt,Es_opt,Peak_new_opt,ins_Cost_opt,grid_opt]=SDP_policy(t,Es, Peak) % Peak: Peak has been recorded so far in the receding horizon
...
for j=1:Length_controls %% Loop on control
    ...
    Peak_new=max(Peak,grid_corrected); % Peak: Peak has been recorded so far in the receding horizon
    ...
    if (total_cost < opt) % If yes update the optimal decision
