...
demand_=Load_Forecast_/1000;% Convert to [kW]
...
cvx_begin quiet
    ...
    minimize sum(max(grid,0))*rate+rate_max.*max(max(grid),Peak);% max(max(grid),Peak): Peak is the Target Peak (Which is updated every one step)
