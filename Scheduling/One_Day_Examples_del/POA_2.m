...
temp=cosd(zenith).*cosd(x(1))+sind(zenith).*sind(x(1)).*cosd(azimuth-x(2));
...
Ediff(incident_angle<90)=DHI(incident_angle<90) .*(1+ cosd(incident_angle(incident_angle<90))) .* 0.5+GHI(incident_angle<90).*(0.012.*zenith(incident_angle<90)-.04).*(1-cosd(incident_angle(incident_angle<90)))./2;% Diffuse Radiation (Sky&Ground) 
