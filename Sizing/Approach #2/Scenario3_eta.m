...
cvx_begin
    ...
    variables Q1(N) Q2(N)  Cb N_PV grid_neg(N) grid_pos(N);  % grid_pos: grid>0, grid_neg: grid<0
    ...
    grid=Load'-N_PV*PV'+Q1/eta-Q2*eta;
    ...
    LCC=LCC(N_PV,Cb,(Q1-Q2));% Life cycle cost is function of the (1) sizes (N_PV, Cb) and (2) the charging rate (Q) as Q determines the sizes of the inverter and converter
    
    % Objective Function
    minimize sum(grid_pos*step/4)/(1000)*rate+(sum(grid_neg*step/4)/(1000)*(-0.01))+rate_max*sum(Peak_after_matrix)/1000+LCC/20
