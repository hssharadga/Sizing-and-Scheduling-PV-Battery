...
cvx_begin
    ...
    LCC=LCC_(N_PV,Cb,Q,y)% Life cycle cost is function of the (1) sizes (N_PV, Cb) and (2) the charging rate (Q) as Q determines the sizes of the inverter and converter
    
    % Objective Function
    minimize (sum(grid_*step/4)/(1000)*rate+rate_max*sum(Peak_after_matrix)/1000+LCC/20/6)
    ...
    % N_PV>=0;% The size is nonnegative   updated in #1
    ...
    % N_PV>=1000-M*B;
    % N_PV<=1000+M*(1-B);
    % #2 is updated as follow 
    y>=1000*B-M*B;
    y<=1000*B;
    N_PV-y>=1000-1000*B;
    N_PV-y<=1000-1000*B+M*(1-B);
