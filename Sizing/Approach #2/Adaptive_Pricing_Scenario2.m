...
cvx_begin
    ...
    variables Q(N) Cb N_PV y; % Cb in [kWh]
    variable  B binary
    ...
    L_C_C=LCC_(N_PV,Cb,Q,y);% Life cycle cost is function of the (1) sizes (N_PV, Cb) and (2) the charging rate (Q) as Q determines the sizes of the inverter and converter
    
    % Objective Function
    minimize (sum(grid_*step/4)/(1000)*rate+rate_max*sum(Peak_after_matrix)/1000+L_C_C/20/6)
