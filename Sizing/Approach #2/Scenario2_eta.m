% Scenario 2 but charging/discharging is not ideal (eta<>1)
...
cvx_begin
    ...
    grid_=Load'-N_PV*PV'+max(eta*Q,Q/eta);
    ...
    LCC=LCC(N_PV,Cb,Q)% Life cycle cost is function of the (1) sizes (N_PV, Cb) and (2) the charging rate (Q) as Q determines the sizes of the inverter and converter
    
    % Objective Function
    minimize (sum(grid_*step/4)/(1000)*rate+rate_max*sum(Peak_after_matrix)/1000+LCC/20)
